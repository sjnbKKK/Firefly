...
function parseIssueBody(body) {
	...
	for (const line of lines) {
		...
		if (/[:：]/.test(line)) {
			...
			else if (/网站链接|站点链接|链接|网址|地址/.test(key))
				assignField("site_url", value);
			else if (/友链页面|友链地址/.test(key))
				assignField("friend_page_url", value);
...
function extractTags(block) {
	...
	const tags = [...match[1].matchAll(/["']([^"']+)["']/g)].map(
		(item) => item[1],
	);
...
function parseFriendsConfig(content) {
	...
	if (!listMatch) {
		throw new Error(
			"未找到 friendsConfig 数组，请检查 src/config/friendsConfig.ts 的格式。",
		);
	}

	const friendBlocks = [
		...listMatch[1].matchAll(/\{([\s\S]*?)\}(?=\s*,|\s*$)/g),
	].map((item) => item[1]);
	if (!friendBlocks.length) {
		throw new Error(
			"未解析到任何 friendsConfig 条目，请检查 src/config/friendsConfig.ts 的格式。",
		);
	}
...
function updateFriendsConfig(repoRoot, data, issueId) {
	...
	const nextFriend = {
		...
		imgurl:
			data.site_avatar ||
			`https://api.dicebear.com/7.x/identicon/svg?seed=${encodeURIComponent(data.site_name)}`,
		desc: data.site_desc || "",
...
function formatFriendsConfig(repoRoot) {
	...
	run(biomeBinary, ["format", "--write", FRIENDS_CONFIG_RELATIVE_PATH], {
		cwd: repoRoot,
	});
}
...
async function validateFriendPage(pageUrl) {
	...
	try {
		...
		const backlinkHost = trimTrailingSlash(SITE_INFO.url)
			.replace(/^https?:\/\//, "")
			.toLowerCase();
		const hasBacklink =
			joined.includes(backlinkHost) ||
			joined.includes(SITE_INFO.name.toLowerCase());
...
function commitAndPush(repoRoot, defaultBranch, filePath, siteName) {
	...
	const hasChanges =
		spawnSync("git", ["diff", "--cached", "--quiet", "--", filePath], {
			cwd: repoRoot,
			shell: false,
		}).status !== 0;
...
module.exports = async function processFriendRequest({ github, context }) {
	...
	try {
		...
		if (
			!normalizeUrl(formData.site_url) ||
			!normalizeUrl(formData.friend_page_url)
		) {
			await createComment(
				github,
				owner,
